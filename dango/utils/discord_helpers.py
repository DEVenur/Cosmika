...
SYSINFO_MARKER = "> [dango-sysinfo]"

_REPLY_QUOTE_MAX = 150

USER_MENTION_RE = re.compile(r"<@!?(\d+)>")
ROLE_MENTION_RE = re.compile(r"<@&(\d+)>")


def resolve_mentions(content: str, mention_map: dict[str, str]) -> str:
    """Replace Discord mention tokens (<@ID>, <@!ID>, <@&ID>) with display names."""
    if not mention_map or not content:
        return content
    for token, name in mention_map.items():
...
def format_reply_context(
    ...
    """Synthesize a Discord reply reference into the current message content.

    call_agent prepends 'Author: ' separately, so this returns only the content portion.
    """
    quoted = ref_content.replace('"', "'")
    if len(quoted) > _REPLY_QUOTE_MAX:
        quoted = quoted[:_REPLY_QUOTE_MAX] + "…"

    if current_author == ref_author:
        prefix = f'(earlier: "{quoted}")'
    else:
        prefix = f'{ref_author} said "{quoted}"'

    return f"{prefix} {current_content}" if current_content else prefix


def format_sysinfo(text: str) -> str:
    """Wrap text in a dango-sysinfo blockquote. Visible to users, stripped from LLM history."""
    lines = "\n".join(f"> {line}" for line in text.splitlines())
    return f"{SYSINFO_MARKER}\n{lines}"
